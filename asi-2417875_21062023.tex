...
\end{adjustwidth}
\caption{Topographic %MDPI: non-English is not allowed in the image, please replace it or ensure it will not affect understanding. % Author's reply: the map in Figure 1 is redone with replaced names from French to English. Following corrections are done:  Sétif --> Setif; Béchar --> Bechar; Béjaïa --> Bejaia; Tébessa --> Tebessa; Grand Erg Oriental --> Great Eastern Sand Sea; Grand Erg Occidental --> Great Western Sand Sea; Kçour --> Ksour; Ouled Naïl --> Ouled Nail; Tademaït --> Tademait; M'zab --> Mzab.
...
The data used in this study consisted of nine (9) consecutive Landsat satellite scenes spanning from 2014 to 2022, with scenes taken in January on the following exact dates: (a%MDPI: Please confirm if the bold is unnecessary and can be removed. The following highlights are the same. % Author's reply: yes, can be removed (I removed it).
) 1 January 2014, %MDPI: please check if the date format should be changed to English format, such as 1 January 2014. % Author's reply: yes, changed to English format, as suggested.
 (b) 4 January 2015, (c) 7 January 2016, (d) 25 January 2017, (e) 28 January 2018, (f) 15 January 2019, (g) 2 January 2020, (h) 4 January 2021, (i) 15 January 2022. Through a visual inspection of the images, it is possible to see the dynamics of the salinity on the ephemeral salt lakes where bright cyan regions signify salted regions, {while dark blue areas indicate water, brownish and beige colors indicate bare soil and sands with diverse landforms, and green areas indicate vegetation areas}. 
...
Figure \ref{fig05} shows the outline of the general structure of the libraries within the GRASS GIS framework for image processing. 
\vspace{-4pt}
\begin{figure}[H] 
	\centering
		\includegraphics[width=13.8cm]{Fig_05}
	\caption{The outline of the general structure of the GRASS GIS framework. The hierarchical structure of the image processing process with the functionality of the key modules of the GRASS GIS and sequential workflow of the data processing. Flowchart made in R library 'mermaid'. Source: author.} \label{fig05}
\end{figure} 

The workflow of the GRASS GIS includes several modules that handle remote sensing data: g.region, i.group, i.cluster, i.maxlik, d.rast.leg, and d.mon wx0. The GRASS GIS has several modules that actually process the data; the content and connections of these processes are presented in Figure \ref{fig05}. 
...
\begin{enumerate}
\itemsep0em
	\item USGS---data: %MDPI: Please confirm if the italics is unnecessary and can be removed. please check all italic format. % Author's reply: yes, can be removed (I removed it).
	...
	\begin{enumerate}
		\item EarthExplorer: The EarthExplorer repository was used to obtain and download the imagery; it is a powerful resource for satellite {images and }remote sensing data management, and is a storage place for large datasets of Landsat 8--9 OLI/TIRS images.
		\item Image capture: Raster images were downloaded using a path to the Landsat files located in the EarthExplorer repository. The images were fetched by accessing the USGS open-access files, and stored in a working folder of the GRASS GIS.		
	\end{enumerate}
	\item GMT---mapping: The map of the study area in Algeria, showing the exact location of the Landsat images, was plotted using GEBCO/SRTM raster grids{ through} {conventional }cartographic methods.	
	\item DCW---adding annotations: The Digital Chart of the World (DCW) Gazetteer database was utilized within the cartographic workflow to trace, mark, and add annotations to the map of Algeria. The DCW data, including annotation features derived from 1:1,000,000 data, were applied to Algeria. Generalized annotations were added to the maps using coordinates.	
	\item GRASS GIS---image processing: The images were processed in the GRASS GIS using a a sequence of modules: `g.region', `i.group', `i.cluster', `r.support', `i.maxlik', `d.mon', `d.legend'.	
		\begin{enumerate}
			\item `i.group'---detection of the groups: The group and subgroups were selected by identifying visible bands in the Landsat collection and {OLI/TIRS} sensor (8 or 9). This improved the time of computing and optimized the use of {computer }memory {(M1 chip 2020 of the MacBook Air)} during image processing.  
			\item `r.support'---semantic labeling: Semantic labels were assigned using the `r.support' module for repeatability of regularly used images in the time series. The labels were assigned using the Landsat sensor OLI for each band correspondingly, using the `map' function (e.g., map = rastername). Annotating the labels was done using the `r.support' module in GRASS GIS.
			\item `i.cluster'---grouping: Each cluster on the image was generated via {the k-means clustering} algorithm, configured as a loop of {codes} processing the image by the GRASS GIS. This resulted in 10 defined clusters visualized as groups of pixels on the images. This procedure was performed automatically through k-means clustering using the GRASS GIS `i.cluster' module. The following parameters were tested: the input imagery group, the input imagery subgroup (30 m visible Landsat bands), and the signature file containing threshold levels for the assignment of the pixels based on the intensity of brightness, and according to the spectral reflectance (level of color in pixels considered or ignored).
			\item `i.maxlik'---classification: The classification of the Landsat {8--9} OLI/TIRS images was applied to each scene with extracted bands in the visible spectrum (bands 1 to 7 with a resolution of 30 m). The classes on the images were adjusted using the cycles of the algorithm executed iteratively until only the 10 classes of the land cover types {remained}.
			\item Analysis: The validation of errors was performed by the computed rejection probability values with pixel classification confidence levels for each map. This was necessary for the analysis of the classification {accuracy of} the {images, covering }Chotts Melrhir and Merouane Lakes, where neighboring pixels cross each other in similar classes.
			 \item `d.mon wx0' and `d.rast.leg'---visualization: The maps were visualized as several images of the land cover types, based on the performed automated classification, and separated by  one-year gaps between each map. The sequence of maps representing the segments of land cover classes was converted into a graphical format. The maps were visualized with legends using the `d.mon' and `d.legend' GRASS GIS modules.	
		\end{enumerate}
...
The numerical pairwise comparison of each pair of images was performed using  a quantitative evaluation of the output raster map holding reject threshold results. This was performed by the chi-squared %MDPI: Please confirm if the italics is unnecessary and can be removed. The following highlights are the same. % Author's reply: yes, italics can be removed (I removed it).
...
\vspace{6pt}
\funding{The %MDPI: Information regarding the funder and the funding number should be provided. Please check the accuracy of funding data and any other information carefully. % Author's reply: yes, correct.
...
\newblock {\em J. Afr. Earth Sci.} {\bf 2017}, {\em
  127},~146--158. %MDPI: we removed the extra information, please confirm. same meanings for below % Author's reply: yes, confirm.
...
\newblock Chapter 17 Geomorphometry in GRASS GIS. {\em Dev. Soil Sci. %MDPI: we changed it to journal type and removed extra information, please confirm. % Author's reply: yes, confirm.
}
...
%MDPI: Refs. 32 and 65 are duplicated. Please remove duplicated references and rearrange all the references to appear in numerical order. Please ensure that there are no duplicated references. % Author's reply: Ref. 32 (Hacinietal2006) is deleted, also its citation in the text. Yes, all the references are in numerical order.
...
\newblock Updated geochemical and isotopic data from the Continental
  ...
  Algeria).
\newblock {\em Quat. Int.} {\bf 2012}, {\em 257},~64--73. % Author's reply: yes, confirm.
...
\newblock {\em Energy Procedia} {\bf 2019}, {\em 157},~59--67. % Author's reply: yes, confirm.
...
\newblock {\em Energy Procedia} {\bf 2017}, {\em 119},~228--235. % Author's reply: yes, confirm.
...
\newblock In Proceedings of the Well Evaluation Conference Algeria, Schlumberger Houston, TX, USA, 25 March 1995; %MDPI: Please add the conference date (day month year). % Author's reply: added information.
\mbox{ pp. 1--93.}
...
\newblock {\em Tectonophysics} {\bf 2009}, {\em 475},~9--28. % Author's reply: yes, confirm.
...
\newblock Ph.D. Thesis, Université Paris-Sud. Paris, France, %MDPI: please add the name of Degree-Granting University. % Author's reply: added information: Université Paris-Sud
 1995.
...
\newblock Retrieval and interpretation of precipitation rates generated from
  the composition of the Merouane Chott ephemeral lake. % Author's reply: double reference is deleted.
...
%\newblock Doctorat  %MDPI: please check if the information should be kept. % Author's reply: yes, can be removed (I removed it).
...
\newblock {\em J. Afr. Earth Sci.} {\bf 2015}, {\em
  112},~471--484. % Author's reply: yes, confirm.
...
\newblock {\em Mater. Today Proc.} {\bf 2022}, {\em 51},~2105--2108. % Author's reply: yes, confirm.
...
\newblock {\em G{\'e}ologie du Sahara Occidental et Central}; Tervuren African
  Geoscience Collection, Royal Museum for Central Africa (Musée royal d’Afrique centrale). Tervuren, Belgium, %MDPI: Please provide more information about the article type, such as book (please provide the name and location of the publisher); online resource (please provide the URL of the website and the date it was accessed (Date Month Year)); or journal article (please provide the name of the journal, the year and volume in which it was published, and the page number). Please refer to https://www.mdpi.com/authors/references for full reference formatting guides. 
  % Author's reply: this is a book. Information is added, obtained from the website: https://www.africamuseum.be/sites/default/files/media/docs/research/publications/rmca/online/digital-publications-geology/Sahara.pdf The name and location of the publisher: name: Royal Museum for Central Africa (Musée royal d’Afrique centrale), location (city, country): Tervuren, Belgium.
  2005.
...
\newblock In Proceedings of the L'hydraulique Souterraine. Compte Rendu des
  Sixièmes Journées de L'hydraulique, Nancy, France, 28--30 June 1960. %MDPI: we removed extra information, please confirm. % Author's reply: yes, confirm.
%  Tome 2, 1961.,
...
\newblock {\em Geophys. Res. Lett.} {\bf 2003}, {\em 30},~1--4. %MDPI: please add page number if possible. % Author's reply: added pages (1--4).
