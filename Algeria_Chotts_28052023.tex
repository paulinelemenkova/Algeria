...
\hl{All the images were selected on January period, for two reasons. First, the water level in chotts is fluctuating with highest level during wet period (i.e., spring and winter) when the lakes are filled by water from the occasional rainwater or groundwater sources. In contrast, during summer and autumn periods, the majority of the lakes are completely dry due to the highest temperature and evaporation rates in Sahara desert. Therefore, to detect changes on the images, it is necessary to select the wet period in the calendar year. Second reason was related to the quality of the data covering target area related to the cloudiness of the images. Many scenes of the Landsat 8-9 OLI/TIRS were checked, and only those with low cloud coverage (below 10\%) were selected for classification. Such conditions corresponded to the images taken on January during the period of 2014--2022, Figure }\ref{fig03}.
...
%		\centering
			\hspace{60pt}\includegraphics[width=0.87\textwidth]{Fig_03a.jpg}
		\caption{2014}
	\end{subfigure}%
	\begin{subfigure}[b]{.40\textwidth}
%		\centering
			\hspace{60pt}\includegraphics[width=0.87\textwidth]{Fig_03b.jpg}
		\caption{2015}
	\end{subfigure}%
	\begin{subfigure}[b]{.40\textwidth}
%		\centering
			\hspace{60pt}\includegraphics[width=0.87\textwidth]{Fig_03c.jpg}
		\caption{2016}
	\end{subfigure}%
}\\
\vfill \vspace{1mm}
\makebox[0.90\linewidth][r]{%
	\begin{subfigure}[b]{.40\textwidth}
		%\centering
			\hspace{60pt}\includegraphics[width=0.87\textwidth]{Fig_03d.jpg}
		\caption{2017}
	\end{subfigure}%
	\begin{subfigure}[b]{.40\textwidth}
		%\centering
			\hspace{60pt}\includegraphics[width=0.87\textwidth]{Fig_03e.jpg}
		\caption{2018}
	\end{subfigure}%
	\begin{subfigure}[b]{.40\textwidth}
		%\centering
			\hspace{60pt}\includegraphics[width=0.87\textwidth]{Fig_03f.jpg}
		\caption{2019}
	\end{subfigure}%
}\\
\vfill \vspace{1mm}
\makebox[0.90\linewidth][r]{%
	\begin{subfigure}[b]{.40\textwidth}
		%\centering
			\hspace{60pt}\includegraphics[width=0.87\textwidth]{Fig_03g.jpg}
		\caption{2020}
	\end{subfigure}%
	\begin{subfigure}[b]{.40\textwidth}
		%\centering
			\hspace{60pt}\includegraphics[width=0.87\textwidth]{Fig_03h.jpg}
		\caption{2021}
	\end{subfigure}%
	\begin{subfigure}[b]{.40\textwidth}
		%\centering
			\hspace{60pt}\includegraphics[width=0.87\textwidth]{Fig_03i.jpg}
		\caption{2022}
	\end{subfigure}%
}
...
\begin{lstlisting}[language=bash,caption=GRASS GIS code for atmospheric correction of the reflectance values of pixels (here: a case of the image on 2014 repeated likewise for all other images on different years).,style=mystyle,label={lst01}]
...
\hl{In this section, we evaluate the results and evaluate the performance of the classification algorithms on the nine images. The evaluation is performed using pixel classification confidence levels using rejection probability values. The process of classification has a flaw related to the assignment of pixels to the classes using their spectral signatures. The allocation of pixels may be different from one year to another due to different nuances in spectral signatures caused by the wetland brightness, greenness and other remote sensing characteristics caused by the level of chlorophyll in leaves. Besides, the sand dust creating dunes on the surface cause selected pixels to be assigned to neighbouring classes such as non-consolidated and consolidated soils.}

The qualitative evaluation of the water level coverage in the Melrhir and Merouane Chotts on the satellite images for each Landsat scene is based on the number of land cover classes as landscape segments, Figure \ref{fig06}. \hl{The GRASS GIS framework of image classification is developed considering a principle of confidence. Consequently, it was adapted to the recognition of the dominant land cover classes by selecting landscape patches with homogeneous spectral reflectance of pixels within each cluster group. In order to evaluate the classification results objectively, we test all implemented algorithms of the GRASS GIS on nine classified images for comparative analysis of classification results with different probability values of pixels that belong to the identical land cover classes in different years and showed in greyscale plots for each pair of images in Figures }\ref{fig06}, \ref{fig07}, \ref{fig08} and \ref{fig09}.

\hl{The numerical pairwise comparison of each pair of images was performed using quantitative evaluation of the output raster map holding reject threshold results. This is performed by the \emph{chi-squared} test and showed as the greyscale monochrome image for the maps on all corresponding years. Using this contingency test for each pair of images, the data were verified by the statistical hypothesis test adopted to the large sample sizes of each 16-bit Landsat scene. Hence, for each image,  the test evaluated the independency of the two categorical variables corresponding to the two dimensions of land cover classes.}
